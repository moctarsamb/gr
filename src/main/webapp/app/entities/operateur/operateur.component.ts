...
export class OperateurComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.operateurService
                ...
                .subscribe(
                    (res: HttpResponse<IOperateur[]>) => this.paginateOperateurs(res.body, res.headers),
    ...
    registerChangeInOperateurs() {
        this.eventSubscriber = this.eventManager.subscribe('operateurListModification', response => this.reset());
    }
    ...
    protected paginateOperateurs(data: IOperateur[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.operateurs.push(data[i]);
        }
    }
